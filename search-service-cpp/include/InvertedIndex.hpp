...
public:
    ...
    // Busca conjuntiva (AND): a entidade precisa casar com todos os tokens pesquisados
    // e ter capacidade suficiente (capacity_adults >= minAdults)
    std::vector<std::string> searchConjunctive(
        const std::vector<std::string>& queryTokens,
        std::optional<int> minAdults = std::nullopt
    ) const;

    const CatalogEntity* getEntity(const std::string& id) const;

    // Retorna quais dos tokens da busca estao presentes no catalogo
    std::vector<std::string> getMatchedTokens(const std::vector<std::string>& queryTokens) const;
