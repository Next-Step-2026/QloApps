...
#define ASSERT_TRUE(condition, message) \
    do { \
        if (!(condition)) { \
            std::cerr << "FAIL: " << message << " (" << #condition << ") na linha " << __LINE__ << std::endl; \
            std::exit(1); \
        } \
    } while (0)
...
#define ASSERT_EQ(val1, val2, message) \
    do { \
        if ((val1) != (val2)) { \
            std::cerr << "FAIL: " << message << " | Esperado: " << (val2) << " | Obtido: " << (val1) \
                      << " na linha " << __LINE__ << std::endl; \
            std::exit(1); \
        } \
    } while (0)
...
void testNormalization() {
    ...
    ASSERT_EQ(normalized, "suite vista mar com banheira & piscina climatizada!", "Normalizacao basica de acentos e caixa");

    auto tokens = Normalizer::tokenize(text, true);
    // Stopwords como "com" devem ser removidas
    std::vector<std::string> expected = {"suite", "vista", "mar", "banheira", "piscina", "climatizada"};
    ASSERT_EQ(tokens.size(), expected.size(), "Quantidade de tokens apos stopwords");
...
void testExtractor() {
    ...
    ASSERT_TRUE(!f1.amenities.empty() && f1.amenities[0] == "vista_mar", "Deve identificar comodidade vista_mar");
    ...
    ASSERT_TRUE(!f2.amenities.empty() && f2.amenities[0] == "banheira", "Deve identificar banheira");
    ...
    ASSERT_TRUE(!f4.adults.has_value(), "Numero solto 104 nao pode ser interpretado como adultos");
...
// 3. Teste de Correspondencia Conjuntiva AND e Filtro de Capacidade (RN-003, RN-004)
void testConjunctiveSearchAndCapacity() {
    ...
    // Busca "quarto" com minAdults = 2: room-sgl-03 (capacidade 1) deve ser excluido
    auto tokens3 = Normalizer::tokenize("quarto");
    auto res3 = index.searchConjunctive(tokens3, 2);
    ASSERT_EQ(res3.size(), 1, "Apenas o quarto standard casal tem capacidade >= 2");
...
int main() {
    std::cout << "=== Executando Testes Unitarios do Motor de Busca C++ (QLO-FEAT-008) ===" << std::endl;

    testNormalization();
    testExtractor();
    testConjunctiveSearchAndCapacity();

    std::cout << ">>> TODOS OS TESTES PASSARAM COM SUCESSO (100% OK)! <<<" << std::endl;
    return 0;
}
