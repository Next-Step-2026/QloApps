...
std::vector<std::string> InvertedIndex::getMatchedTokens(const std::vector<std::string>& queryTokens) const {
    ...
    for (const auto& id : insertionOrder_) {
        if (candidateIds.find(id) != candidateIds.end()) {
            ...
            if (entIt != entities_.end()) {
                if (minAdults.has_value() && entIt->second.capacityAdults < minAdults.value()) {
