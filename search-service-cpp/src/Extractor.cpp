...
ExtractedFilters Extractor::extract(const std::string& normalizedQuery) {
    ...
    // Regex para adultos/pessoas/hospedes: "2 adultos", "para 3 pessoas", "1 hospede"
    std::regex adultsRegex(R"((\d+)\s*(adultos?|pessoas?|hospedes?|guests?))", std::regex_constants::icase);
    std::smatch match;
...
std::string Extractor::stripFilterPhrases(const std::string& normalizedQuery) {
    // Remove as frases de contagem de pessoas para nao buscarem "2" ou "adultos" no indice
    std::regex filterRegex(R"((?:para|pra)?\s*\b\d+\s*(adultos?|pessoas?|hospedes?|guests?)\b)", std::regex_constants::icase);
