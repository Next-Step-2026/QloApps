...
int main() {
    ...
    svr.Post("/v1/search/parse", [](const httplib::Request& req, httplib::Response& res) {
        ...
        std::string correlationId = req.has_header("X-Correlation-ID")
            ...
            : "corr-" + std::to_string(std::chrono::duration_cast<std::chrono::microseconds>(
        ...
        try {
            ...
            std::string strippedQuery = Extractor::stripFilterPhrases(normalizedQuery);
            std::vector<std::string> queryTokens = Normalizer::tokenize(strippedQuery, true);
            ...
            std::vector<std::string> matchedTokens = index.getMatchedTokens(queryTokens);

            // 6. Busca conjuntiva (AND search) com filtro de capacidade (RN-003, RN-004)
            std::vector<std::string> matchingIds = index.searchConjunctive(queryTokens, filters.adults);
            ...
            response["extracted_filters"] = {
                {"adults", filters.adults.has_value() ? json(filters.adults.value()) : json(nullptr)},
    ...
    // Host e porta de escuta configuraveis (padrao 0.0.0.0 para atender host e Docker)
    ...
    if (!svr.listen(host, port)) {
        std::cerr << "Erro fatal: nao foi possivel abrir o servidor HTTP em " << host << ":" << port << std::endl;
        return 1;
    }
